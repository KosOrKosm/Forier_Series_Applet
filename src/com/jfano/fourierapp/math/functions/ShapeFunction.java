...
/**
 * Creates a function F from a shape such that F has domain [0, 1] and outputs complex numbers
 * representing points along the perimeter of the shape, fully tracing the shape if every value of F
 * were to be drawn at once.
 * <p>
...
public class ShapeFunction implements ComplexTimeFunction {
  ...
  /**
   * Initialize this shape function from a <code>PathIterator</code>. The generated function will
   * trace the path of the <code>PathIterator</code>.
   *
  ...
  /**
   * Initialize this shape function from a <code>Shape</code>. The generated function will trace the
   * path of the <code>Shape</code>.
   *
  ...
  /**
   * Initialize this shape function from a SVG file. Configures the function such that it traces the
   * path contained within the SVG file.
   *
  ...
  /**
   * Build this <code>ShapeFunction</code> from the given <code>PathIterator</code>. The generated
   * function will trace the path of the <code>PathIterator</code>.
   * <br><br>
   * This is done by deconstructing the <code>PathIterator</code> into its component equations, then
   * "gluing" those equations together to form a single function.
   *
    ...
    while (!points.isDone()) {
      ...
      switch (points.currentSegment(buffer)) {
        ...
        case PathIterator.SEG_LINETO:
          ...
          dynLines
              .add(new LineTimeEquation(last.getX(), last.getY(), current.getX(), current.getY()));
        ...
        case PathIterator.SEG_QUADTO:
          ...
          dynLines.add(new QuadCurveTimeEquation(last.getX(), last.getY(), buffer[0], buffer[1],
              current.getX(),
              current.getY()));
        ...
        case PathIterator.SEG_CUBICTO:
          ...
          dynLines.add(new CubicCurveTimeEquation(last.getX(), last.getY(), buffer[0], buffer[1],
              current.getX(),
